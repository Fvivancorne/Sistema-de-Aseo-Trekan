...
function Familias({ familias, reload, addToast }) {
  ...
  async function saveFamily() {
    ...
    setSaving(true);
    if(editing){
      const {error} = await supabase.from("familias").update({nombre:form.nombre.trim(),activa:form.activa}).eq("id",editing.id);
      if(error){addToast("Error: "+error.message,"error");}else{addToast("Familia actualizada");}
    } else {
      const {error} = await supabase.from("familias").insert({nombre:form.nombre.trim(),activa:form.activa,participaciones_totales:0,ausencias:0,participaciones_por_mes:{}});
            ...
            return (
              <Card key={f.id}>
                ...
                <div style={{ display:"flex", gap:8, marginTop:16, paddingTop:14, borderTop:"1px solid #F1F5F9" }}>
                  <Btn variant="secondary" size="sm" onClick={()=>{setEditing(f);setForm({nombre:f.nombre,activa:f.activa});setShowDialog(true);}} style={{ flex:1 }}><Icon d={Icons.edit} size={13}/> Editar</Btn>
