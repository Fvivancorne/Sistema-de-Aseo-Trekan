...
function Calendario({ semanas, familias, periodos, reload, addToast }) {
  ...
  async function rebalancear() {
    ...
    // Familias nuevas parten con el promedio actual para competir en igualdad
    const totalesConHistorial = active.map(f=>stats[f.id].total).filter(t=>t>0);
    const promedio = totalesConHistorial.length
      ? Math.round(totalesConHistorial.reduce((a,b)=>a+b,0)/totalesConHistorial.length)
      : 0;
    active.forEach(f=>{
      if(stats[f.id].total===0 && promedio>0) stats[f.id].total = promedio;
    });
...
function Familias({ familias, reload, addToast }) {
  ...
  const [form,setForm]               = useState({nombre:"",activa:true,email:"",email2:"",telefono:""});
  ...
  async function saveFamily() {
    ...
    setSaving(true);
    if(editing){
      const {error} = await supabase.from("familias").update({nombre:form.nombre.trim(),activa:form.activa,email:form.email.trim()||null,email2:form.email2.trim()||null,telefono:form.telefono.trim()||null}).eq("id",editing.id);
      if(error){addToast("Error: "+error.message,"error");}else{addToast("Familia actualizada");}
    } else {
      const {error} = await supabase.from("familias").insert({nombre:form.nombre.trim(),activa:form.activa,email:form.email.trim()||null,email2:form.email2.trim()||null,telefono:form.telefono.trim()||null,participaciones_totales:0,ausencias:0,participaciones_por_mes:{}});
    ...
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          ...
          <Btn onClick={()=>{setEditing(null);setForm({nombre:"",activa:true,email:"",email2:"",telefono:""});setShowDialog(true);}}><Icon d={Icons.plus} size={14}/> Nueva</Btn>
        </div>
      </div>
      ...
      {filtered.length===0 ? (
        <Card style={{ textAlign:"center", padding:64 }}>
          ...
          <div style={{ marginTop:16 }}><Btn onClick={()=>{setEditing(null);setForm({nombre:"",activa:true,email:"",email2:"",telefono:""});setShowDialog(true);}}><Icon d={Icons.plus} size={14}/> Agregar Familia</Btn></div>
        </Card>
      ) : (
            ...
            return (
              <Card key={f.id}>
                ...
                <div style={{ display:"flex", gap:8, marginTop:16, paddingTop:14, borderTop:"1px solid #F1F5F9" }}>
                  <Btn variant="secondary" size="sm" onClick={()=>{setEditing(f);setForm({nombre:f.nombre,activa:f.activa,email:f.email||"",email2:f.email2||"",telefono:f.telefono||""});setShowDialog(true);}} style={{ flex:1 }}><Icon d={Icons.edit} size={13}/> Editar</Btn>
      ...
      <Modal open={showDialog} onClose={()=>setShowDialog(false)} title={editing?"Editar Familia":"Nueva Familia"}>
        ...
        <label style={{ fontSize:13, fontWeight:600, color:"#475569", display:"block", marginBottom:6, marginTop:14 }}>Email mamá <span style={{ fontWeight:400, color:"#94A3B8" }}>(recordatorios)</span></label>
        <Input value={form.email} onChange={v=>setForm(p=>({...p,email:v}))} placeholder="mama@correo.com" type="email"/>
        <label style={{ fontSize:13, fontWeight:600, color:"#475569", display:"block", marginBottom:6, marginTop:14 }}>Email papá <span style={{ fontWeight:400, color:"#94A3B8" }}>(opcional)</span></label>
        <Input value={form.email2} onChange={v=>setForm(p=>({...p,email2:v}))} placeholder="papa@correo.com" type="email"/>
